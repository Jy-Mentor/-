...
import os, sys, gzip, re, warnings, logging, subprocess
...
# 路径配置
# ============================================================
IRON_AGING_DIR = Path(r"c:\Users\Jy-Mentor-7\Desktop\铁衰老")
BASE_DIR = Path(r"c:\Users\Jy-Mentor-7\Desktop\你信我 这不是重蹈覆辙")
L1_OUTPUT_DIR = BASE_DIR / "ferro_aging_ciri_results"
L2_OUTPUT_DIR = IRON_AGING_DIR / "L2_results"
L2_WGCNA_INPUT = IRON_AGING_DIR / "L2_WGCNA_input"
L2_WGCNA_OUTPUT = IRON_AGING_DIR / "L2_WGCNA_output"

for d in [L2_OUTPUT_DIR, L2_WGCNA_INPUT, L2_WGCNA_OUTPUT]:
...
RSCRIPT = r"C:\R\R-4.5.2\bin\Rscript.exe"
...
DISCRIMINATOR_GENES = ["GPX4", "ACSL4", "PTGS2", "HMOX1", "TFRC", "CDKN1A", "IL6"]
...
def load_expression_matrix(ds_name: str) -> Tuple[pd.DataFrame, List[str], List[str]]:
    ...
    if ds_name == 'GSE16561':
        ...
        if counts_file and counts_file.exists():
            expr_gene = pd.read_csv(str(counts_file), sep='\t', index_col=0, compression='gzip')
        else:
        ...
        control_cols = sorted([c for c in all_cols
    ...
    logger.info(f"  [{ds_name}] 加载完成: {expr_gene.shape}, case={len(case_cols)}, ctrl={len(control_cols)}")
...
def run_r_wgcna():
    ...
    result = subprocess.run(cmd, capture_output=True, text=True,
                            cwd=str(IRON_AGING_DIR), timeout=3600,
...
def acsl4_expression_analysis(expr_gene, ds_name, case_cols, control_cols, dual_scores):
    ...
    species = 'Human' if ds_name in ['GSE16561', 'GSE37587'] else \
              'Rat' if ds_name in ['GSE61616', 'GSE97537'] else 'Mouse'
...
def gpx4_trend_analysis(expr_gene, ds_name, dual_scores):
    ...
    q75, q50, q25 = valid_idsp.quantile(0.75), valid_idsp.quantile(0.50), valid_idsp.quantile(0.25)
    available_cols = set(expr_gene.columns)
    ...
    if n_high >= 2 and n_low >= 2:
        gpx4_high_vals = expr_gene.loc["GPX4", [c for c in high_isp if c in available_cols]].values.astype(float)
        gpx4_low_vals = expr_gene.loc["GPX4", [c for c in low_isp if c in available_cols]].values.astype(float)
    ...
    return {
        'dataset': ds_name,
        'species': species,
        'gpx4_high_isp': float(gpx4_high_mean) if not np.isnan(gpx4_high_mean) else np.nan,
        ...
        'gpx4_high_std': float(gpx4_high_std) if not np.isnan(gpx4_high_std) else np.nan,
...
def analyze_all_datasets():
    ...
    for ds_name in DATA_DIRS:
        try:
            ...
            # ACSL4 表达分析
            acsl4_res = acsl4_expression_analysis(expr_gene, ds_name, case_cols, control_cols, dual_scores)
            if acsl4_res:
                all_acsl4.append({k: v for k, v in acsl4_res.items()
            ...
            logger.info(f"  [{ds_name}] ACSL4={'OK' if acsl4_res else 'FAIL'}, GPX4={'OK' if gpx4_res else 'FAIL'}")
    ...
    if all_acsl4_data:
        ...
        acsl4_data_df.to_csv(L2_OUTPUT_DIR / "L2_ACSL4_expression_data.csv", index=False)
    ...
    if overlap_file.exists():
        ...
        for ds_a in jaccard_pivot.index:
            for ds_b in jaccard_pivot.columns:
                if pd.isna(jaccard_pivot.loc[ds_a, ds_b]) and ds_b in jaccard_pivot.index and ds_a in jaccard_pivot.columns:
...
def main():
    ...
    # Step 3: 读取 WGCNA 结果
    wgcna_results = read_wgcna_results()
    ...
    if r_plot_script.exists():
        logger.info("\n===== Step 6: R 绘图 =====")
        result = subprocess.run([RSCRIPT, str(r_plot_script)],
                                capture_output=True, text=True,
                                cwd=str(IRON_AGING_DIR), timeout=600,
                                encoding='utf-8', errors='replace')
        logger.info(result.stdout)
        if result.stderr:
            logger.warning(result.stderr)
