...
# ============================================================
IRON_DIR = _L2_CFG["project_root"]
RAW_TAR = _L2_CFG["datasets"].get("GSE174574")
if RAW_TAR is None or not RAW_TAR.exists():
    # 兼容旧路径
    RAW_TAR = Path(r"D:\反向网络药理学\L1 数据集\RNA-seq\GSE174574_RAW.tar")
EXTRACT_DIR = IRON_DIR / "GSE174574_extracted"
MODULE2_OUT = _L2_CFG["module2_output_dir"]
R_PLOT_DIR = _L2_CFG["module2_plot_dir"]
MODULE3_IFACE = _L2_CFG["module3_interface_dir"]
...
# ============================================================
sys.path.insert(0, str(IRON_DIR))  # noqa: E402
from L1.idsp_gene_sets import (  # noqa: E402
    FERROPTOSIS_ALL,
    PURE_FERROPTOSIS,
    PURE_SENESCENCE,
    SENESCENCE_ALL,
...
def extract_and_load() -> anndata.AnnData:
    ...
    # 解压 (仅当目录不存在或为空)
    need_extract = not EXTRACT_DIR.exists() or not list(EXTRACT_DIR.glob("*.gz"))
    if need_extract:
    ...
    for gsm_name in sorted(gsm_names):
        ...
        try:
            ...
            adatas.append(adata)
            logger.info(
                f"  [{gsm_name}] {adata.n_obs} cells x {adata.n_vars} genes, {condition}"
            )
        except Exception as e:
...
def compute_iron_aging_scores(adata: anndata.AnnData) -> anndata.AnnData:
    ...
    n_ferr, scores_ferr = score_genes_safe(
        adata, list(FERROPTOSIS_ALL), "ferroptosis_score"
    )
    if n_ferr:
        logger.info(f"  铁死亡评分: {n_ferr}/{len(FERROPTOSIS_ALL)} 基因 (V2精修)")

    # --- 衰老评分 (V2: SENESCENCE_ALL = 113 基因) ---
    n_sene, scores_sene = score_genes_safe(
        adata, list(SENESCENCE_ALL), "senescence_score"
    )
    if n_sene:
        logger.info(f"  衰老评分: {n_sene}/{len(SENESCENCE_ALL)} 基因 (V2精修)")

    # --- 桥接共享评分 (V2 新增: SHARED_GENES = 14) ---
    n_shared, scores_shared = score_genes_safe(
        adata, list(SHARED_GENES), "shared_score"
    )
    if n_shared:
    ...
    for sn in score_names:
        ...
        for mn in met_names:
            if mn not in adata.obs:
                continue
            r, p = stats.pearsonr(
                adata.obs[sn].fillna(0).values, adata.obs[mn].fillna(0).values
            )
            corr_rows.append(
                {
                    "score": sn,
    ...
    if corr_rows:
        pd.DataFrame(corr_rows).to_csv(
            MODULE2_OUT / "metabolism_correlations.csv", index=False
        )
...
def cluster_and_annotate(adata: anndata.AnnData) -> anndata.AnnData:
    ...
    if score_names:
        ...
        adata_hvg.obs["cell_type"] = adata_hvg.obs["leiden"].apply(
    ...
    for set_name, gene_set in score_sets.items():
        ...
        for i in range(n_permutations):
            ...
            for ct in cell_types:
                null_dist[ct].append(
                    float(
                        adata.obs.loc[adata.obs["cell_type"] == ct, "_perm_tmp"].mean()
                    )
                )
        ...
        n_sig = sum(
            1 for r in perm_results if r["gene_set"] == set_name and r["significant"]
        )
        logger.info(f"    {set_name}: {n_sig}/{len(cell_types)} CT 显著")

    if perm_results:
        pd.DataFrame(perm_results).to_csv(
            MODULE2_OUT / "iron_aging_significance.csv", index=False
        )
...
def run_liana_lr_analysis(adata_full: anndata.AnnData, adata_hvg: anndata.AnnData):
    ...
    try:
        ...
        if (
            "aggregate_rank_mcao" in merged.columns
            and "aggregate_rank_sham" in merged.columns
        ):
            merged = merged[
        ...
        merged["magnitude_rank_diff"] = (
            merged["magnitude_rank_mcao"] - merged["magnitude_rank_sham"]
        )
...
def differential_and_interfaces(adata_full: anndata.AnnData, adata: anndata.AnnData):
    ...
    if deg_ct_list:
        ...
        # 筛选显著
        deg_sig = deg_all[
            (deg_all["logfoldchanges"].abs() > 0.25) & (deg_all["pvals_adj"] < 0.05)
        ]
    ...
    if adata_full.raw is not None:
        ...
        if raw_pb_matrices:
            ...
            logger.info(
                f"  Raw伪bulk: {raw_pb_df.shape[1]} 组 x {raw_pb_df.shape[0]} 基因"
            )
    ...
    if all(c in adata_full.obs for c in ["isp_index"] + met_names):
        scat_cols = ["isp_index", "cell_type", "condition"] + [
            c for c in met_names if c in adata_full.obs
        ]
...
def save_results(adata: anndata.AnnData, adata_full: anndata.AnnData = None):
    ...
    cols_to_save = obs_cols + [
        c for c in score_cols + aucell_cols + time_cols + isp_cols if c in adata.obs
    ]
    ...
    if all(c in adata.obs for c in consensus_detail_cols):
        consensus_detail = (
            adata.obs.groupby("cell_type")[consensus_detail_cols].mean().round(4)
        )
