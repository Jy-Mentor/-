...
> 报告版本：v1.1  
> 编制日期：2026-06-20  
> 更新记录：v1.1 补充 v3 5-Fold 实验结果与 Node2Vec 数据泄漏修复说明  
...
1. **数据可信**：通过严格训练图构建与 3-Fold 交叉验证，修复了 ct 任务的数据泄漏问题。
2. **实验可比**：新增 MLP 与 Random 基线，证明在当前小图下 GAT 优于 HGT，图结构仍有正向贡献。
3. **生物学可解释**：新增 BCP 靶向铁衰老基因集指标，可量化关键基因排名，但 ACSL4 仍不突出。
4. **工程可维护**：完成 v4.0 数据库驱动分层架构初版，图构建层已可独立运行并通过测试。

所有整改代码已通过质量门禁（`ruff check .` 0 error、`validate_inputs.py` 54/54 OK），系统功能稳定、结果可信，符合项目质量标准与科研诚信要求。
